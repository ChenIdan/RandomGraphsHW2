...
void main(int arc, char *argv[]){
   ...
   fprintf(file2,"Experiment with first heuristic, with n = %d and p=%f\n",n,p);
   ...
   printf("average edges number: %f, probability was: %f, and number of possible edges was: %d\n, so expected number is: %f\n",edges_num_average/num_of_experiments,p,n*(n-1)/2,p*n*(n-1)/2);
